...
const FinancialUsers = () => {
  ...
  const [formData, setFormData] = useState({
    ...
    date: '',
    note: '',
    type: '' // Nuevo campo para el tipo de ingreso
  });
  ...
  const handleEditRow = async (id) => {
    ...
    try {
      await updateDoc(doc(db, 'financialUsers', id), updatedRow);
      setRows([...rows]);
    } catch (e) {
  ...
  return (
    <div className="financial-users">
      <h2>Tabla de donaciones Mensuales Socios</h2>
      <table>
        <thead>
          <tr>
            <th>Nombre</th>
        ...
        <tbody>
          {rows.map((row) => (
            <tr key={row.id}>
              ...
              <td>{row.type}</td>
              <td>
                <button className="edit-button" onClick={() => handleEditRow(row.id)}>Editar</button>
                <button className="delete-button" onClick={() => handleDeleteRow(row.id)}>Eliminar</button>
              </td>
            </tr>
          ))}
        </tbody>
